...
function MenuItemTwo({ cat, sub, head, col, arccol, isSubMenuOpen, toggleSubMenu }) {
    ...
    (product) =>  product.brand === selectedCat
    );
    ...
    return (
      <Container>
        <Names onClick={toggleSubMenu}>
           ...
           <div>{head}</div> <IconColor color={col}> <ExpandMore/></IconColor>

          
        </Names>
        {isSubMenuOpen && (
          <Wrapper>
            <CardArc width="10" height="13" viewBox="0 0 10 11" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              {cat.map((menuItem) => ( 
                <NamesTwo key={menuItem.id} onClick={() => handleCatClick(menuItem)}>
